...
var ocsp = {
  ...
  refresh: function() {
    ...
    exec(`ls ${ca}/ocsp/requests/`, { silent: true }).output.split("\n").forEach(function(filename) {
      ...
      if (test('-f', `${ca}/ocsp/requests/${cached}`)) {
  ...
  serve: function(b64, res) {

    // Get path from b64
    var responsePath = `${ca}/ocsp/responses/${ocsp.filename(b64, true)}`;
...
// Clean cache
exec(`rm -rf ${ca}/ocsp/requests/*`);
exec(`rm -rf ${ca}/ocsp/responses/*`);
exec(`rm -rf ${ca}/ocsp/text/*`);
